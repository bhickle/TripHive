...
- ~~**Builder client multi-city button**~~ — **RESOLVED 2026-06-09 (commit `6d540d3`) as an upsell.** Tracing the flow showed the builder NEVER has a pass at wizard time: a Trip Pass can only be bought against an existing trip (`stripe/checkout` requires a `tripId`, redirects to `/trip/[id]/group`), and the wizard always builds a fresh, pass-less trip. So unlocking the button would let a Free user compose a multi-city trip the server then 403s (`MULTI_CITY_LOCKED`) mid-build. Instead the Free user's "Add another destination" tap now opens a **multi-city-specific** upsell (new `multi_city` UpgradeReason) that names the paid boundary AND leads with the free region-auto-route alternative. **Deferred (bigger flow, not built):** a "buy a Trip Pass, then build a multi-city trip from scratch" path — compose multi-city → create skeleton → buy pass for it → resume build. Touches checkout + builder + build-resume; revisit if the buy-to-plan-from-scratch persona matters at launch.
