...
const discoverItems: DiscoverItem[] = [
  {
    ...
    bookable: true,
    imageGradient: 'from-ocean-700 via-ocean-800 to-earth-700',
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-zinc-100">
        <div className="px-4 md:px-6 py-4 max-w-5xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            ...
            <div className="flex flex-nowrap items-center gap-2 overflow-x-auto w-full sm:w-auto sm:flex-1 sm:min-w-0 -mx-4 px-4 sm:mx-0 sm:px-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {categories.map((cat) => (
                <button
                  key={cat.value}
                ...
                {filteredItems.map((item) => (
                  <div key={item.id} className="bg-white rounded-2xl border border-zinc-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col group">
                    ...
                    <div className="relative h-44 overflow-hidden rounded-t-2xl bg-gradient-to-br from-sky-50 via-zinc-50 to-zinc-100">
                      ...
                      {/* Match score badge */}
                      {item.matchScore >= 90 && (
                        <span className="absolute top-3 right-3 inline-flex items-center px-2.5 py-1 bg-emerald-700/90 backdrop-blur-sm text-white text-xs font-bold rounded-full">
