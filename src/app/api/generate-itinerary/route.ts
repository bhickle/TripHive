...
CROSS-CITY SPLIT DAYS — on day(s) ${crossCityDays.map(d => `Day ${d.dayNumber} (Track A → ${d.trackACity ?? '?'}, Track B → ${d.trackBCity ?? '?'})`).join('; ')} the two tracks spend the WHOLE day in DIFFERENT cities. For each of these days:
- Set the day's "city" to Track A's city, set "trackACity" to Track A's city, and set "trackBCity" to Track B's city.
- Put Track A's venues in track_a with addresses physically in Track A's city; put Track B's venues in track_b with addresses physically in Track B's city. Never mix a Track B (other-city) venue into shared or track_a.
- The groups are apart all day, so DO NOT force a shared dinner: give EACH track its own meals (breakfast/lunch/dinner) in its own city within that track's array, keep "shared" empty (or only a single pre-departure item if they genuinely start together), and set dinnerMeetupLocation to null.
- Include any inter-city transport the user named (e.g. a morning train) as that track's first activity or a transportToNext leg.`
        : '';
      text += `

SPLIT TRACK — USER REQUESTED (honor exactly): The traveler divided specific days into Track A and Track B in their daily outlines. On EVERY day whose outline names "Track A" and "Track B", you MUST split that day: put Track A's activities in track_a, Track B's in track_b, set short trackALabel/trackBLabel descriptors, and use the exact venues and times the user named for each track. For a SAME-CITY split day, keep arrival/shared items in "shared", restaurants in the shared track, and (unless the user says otherwise) both tracks reconvene for dinner — set dinnerMeetupLocation to that day's dinner venue. Days the user did NOT split stay single, shared days.${crossCityNote}`;
...
RESTAURANTS ALWAYS IN SHARED TRACK: All restaurant activities (isRestaurant: true) must ALWAYS be placed in the "shared" track — never in track_a or track_b. Meals are a shared group experience. EXCEPTIONS: (a) a trip explicitly themed around diverging dining preferences (rare); (b) a CROSS-CITY split day (defined below) where the two tracks are in different cities all day — there, each track has its OWN meals inside its own track array, since the groups can't eat together. In all standard itineraries, restaurants go in shared.
