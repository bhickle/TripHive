...
${placesList}`;
    ...
    const priorityContext = Array.isArray(priorities) && priorities.length > 0
      ? `\nTRIP PRIORITIES: ${priorities.join(', ')}. The replacement should feel coherent with these priorities. If the original activity was on a "${track}" track and the trip has specific priorities, prefer a replacement that matches the spirit of those priorities (e.g. a sports track activity should suggest another sporting/active venue, not a museum).`
      : '';
...
- ${mealContext}
- Track: ${track}
- ${budgetContext}${cruiseContext}${priorityContext}
