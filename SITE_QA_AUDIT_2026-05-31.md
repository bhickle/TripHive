...
- [x] **17. Three independent raw `ItineraryDay`-from-Json read paths (NEW, latent). — DONE 2026-06-01** (new `src/lib/itinerary/normalizeDay.ts`; group + print now run loaded days through `normalizeDays()` so `tracks.{shared,track_a,track_b}` are always arrays — closes the latent crash class. The itinerary page keeps its richer `syncAiDays`.) itinerary `syncAiDays` (normalizes `tracks`), group `itineraryDaysData` (`group/Client.tsx:448`, not normalized), print `setDays` (`print/page.tsx:55`). All currently safe via `?.`, but a future `day.tracks.shared.map` without `?.` in group/print will crash. **Fix:** one shared `normalizeDay()` used by all three.

- [x] **18. Triplicated `relativeTime`/`timeAgo` with drift (NEW). — DONE 2026-06-01** (one `relativeTime` in `src/lib/tripDates.ts`; NotificationPanel, admin-support, and notifications-page all import it — the floor/seconds drift is gone). `NotificationPanel.tsx:72` + `admin/support/Client.tsx:54` (Math.round) vs `notifications/page.tsx:45` (Math.floor + seconds "just now" bucket) → same notification reads "1m ago" in the panel and "just now" on the page. **Fix:** one shared helper in `src/lib/`.

- [x] **19. Day-count math duplicated, ceil vs round (NEW). — DONE 2026-06-01** (`computeStatus` + `daysUntil` centralized in `src/lib/tripDates.ts`; dashboard + trips list import them. TripCard had no `computeStatus` copy — audit was slightly off there.) `TripCard.tsx:184` + `dashboard/Client.tsx:310` use `Math.ceil`; `dashboard:370` uses noon-anchored `Math.round` (the batch-2 fix). `computeStatus` is copy-pasted in `dashboard:52`, `trips/Client.tsx:32`, `TripCard.tsx`. Same "N-day trip shows N+1d" class. **Fix:** centralize `daysUntil()`/`tripDayCount()`/`computeStatus()` in `src/lib/tripDates.ts`.
